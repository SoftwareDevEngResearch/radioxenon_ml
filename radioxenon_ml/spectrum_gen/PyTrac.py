...
acceptable_cell_number = [300,400];
#acceptable_cell_number = [800,900];
...
def check_validity(sample):
  ...
  #print(checkExistance(cell_numbers, acceptable_cell_number))
  return (0, 0, 0);
...
def main(args):
    ...
    outAdr = os.getcwd() + '/' + args[2];
    try:
        ignoreLine = int(args[3]);
    except:
        ignoreLine = 10;

    outFile = open(outAdr+'_out.txt', 'w');
    counter_outFile = open(outAdr+'_cnt.txt', 'w');
    ...
    for line in open(adr):
        ...
        if size_counter > 10**8: #Increase this value to make individual file writes larger
            ...
            #outAdr = os.getcwd() + '/' + "file_"+ str(file_count) + args[2];
            outAdr = os.getcwd() + '/' + args[2];
            ...
            if  particle_len > 1:
                if particle_len <= newDataLen:
                    ...
                    if counter % 2 == 1:
                        ...
                        #print ("Particle\n")
                        if NPS%20000 == 0:
                            print ("NPS: " + str(NPS) + ", Particle: "+str(particle))
